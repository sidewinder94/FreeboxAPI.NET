...
public partial class LoginModule : BaseModule
{
    ...
    internal LoginModule(FreeboxApi api) : base (api)
    {
    ...
    public async Task<ApiResponse<AuthorizeResponse>> Authorize()
    {
        var request = new AuthorizeCreationRequest()
        {
            ...
            AppName = this.FreeboxApi.AppInfo.AppName,
            AppVersion = this.FreeboxApi.AppInfo.AppVersion,
            DeviceName = this.FreeboxApi.AppInfo.DeviceName
        };

        var uri = new Uri($"{this.FreeboxApi.ApiInfo.ApiUri}{BaseModuleUri}authorize/");

        return await this.PostAsync<AuthorizeCreationRequest, AuthorizeResponse>(request, uri, bypassAutoLogin: true);
    }
    ...
    public async Task<ApiResponse<AuthorizeProgressResponse>> TrackAuthorization(AuthorizeResponse authorizeResponse)
    {
        if (authorizeResponse == null)
        {
            throw new ArgumentNullException(
                ...
                $"Cannot track the authorization request without at least an Id please provide a {typeof(AuthorizeResponse).Name} object with at least the {nameof(AuthorizeResponse.TrackId)} property filled");
        }

        var uri = new Uri($"{this.FreeboxApi.ApiInfo.ApiUri}{BaseModuleUri}authorize/{authorizeResponse.TrackId}");
        return await this.GetAsync<AuthorizeProgressResponse>(uri, bypassAutoLogin: true);
    }
    ...
    public async Task<ApiResponse<OpenedSession>> SessionOpen(string appSecret = null)
    {
        ...
        var uri = new Uri($"{this.FreeboxApi.ApiInfo.ApiUri}{BaseModuleUri}");

        var challengeResponse = await this.GetAsync<LoginStart>(uri, bypassAutoLogin: true);

        using var hmac = new HMACSHA1(Encoding.UTF8.GetBytes(appToken));
        
        string challengeToReturn = hmac
        ...
        var response = await this.PostAsync<SessionStart, OpenedSession>(sessionStart, new Uri($"{uri}session/"), bypassAutoLogin: true);
    ...
    {
        var uri = new Uri($"{this.FreeboxApi.ApiInfo.ApiUri}{BaseModuleUri}logout/");

        var response = await this.PostAsync<object, EmptyResponse>(null, uri);
